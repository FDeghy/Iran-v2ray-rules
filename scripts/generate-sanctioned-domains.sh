...
gh api https://api.github.com/repos/FDeghy/DynX-AntiBan-Domains/contents/DynX-AntiBan-list.lst -H "Accept: application/vnd.github.raw" > sanctioned-raw.txt
...
cat sanctioned-raw.txt | sed -e 's/^\(|\|\*\|\.\|\-\|0\.0\.0\.0\|127\.0\.0\.1\)*//g' -e 's/\^.*$//g' -e '/!\|?\|@\|#\|\*\|_\|\\\|\/\|\[\|]\|\[\|\([0-9]\{1,3\}\.\)\{3\}[0-9]\{1,3\}/d' -e '/\.$/d' -e '/^\s*$/d' | awk '{$1=$1};1' | perl -CSDA -pe 's/[\x{200C}\x{200D}]//g' | dos2unix | idn2 --no-alabelroundtrip --no-tr46 | LC_ALL=C sort -u > sanctioned-temp.txt
